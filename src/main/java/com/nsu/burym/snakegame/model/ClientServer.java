    ...
    public void update(String eventType, DatagramPacket receivedPacket) {
        try {
            ...
            if (gameMessage.hasState()) {
                ...
                if (roleChangeMsg.hasReceiverRole() && roleChangeMsg.hasSenderRole()) {
                    System.out.println("ROLE SENDER " + roleChangeMsg.getSenderRole());
                    System.out.println("ROLE RECIEVER " + roleChangeMsg.getReceiverRole());
                    ...
                    roleChangeMsg.getReceiverRole() == SnakesProto.NodeRole.DEPUTY) {
                        ...
                        System.out.println("Принял сообщение о том, что мне необходимо стать сервером. Теперь яяяяя: " + roleChangeMsg.getSenderRole());
                        ...
                        if (gameModel.getClientServer() != null) {
                            ...
                            System.out.println("Получил сообщение о том, что у нас новый сервер c портом: " + receivedPacket.getPort());
                        }
                    }
                }
