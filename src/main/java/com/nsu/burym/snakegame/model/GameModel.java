package com.nsu.burym.snakegame.model;

import com.nsu.burym.snakegame.model.announcement.AnnouncementReceiver;
import com.nsu.burym.snakegame.model.announcement.AnnouncementSender;
import com.nsu.burym.snakegame.model.announcement.IpAddress;
import com.nsu.burym.snakegame.model.events.EventManager;
import com.nsu.burym.snakegame.model.exceptions.WrongArgException;
import com.nsu.burym.snakegame.Main;
...
public class GameModel {
    ...
    public synchronized void changeDeputy() throws IOException, InterruptedException {
        System.out.println("CHANGE DEPUTY");
                        ...
                        if (!delete) {
                            ...
                            for (int k = 0; k < players.size(); k++) {
                                if (newSnakes.get(i).getPlayerId() == players.get(k).getId()) {
                                    ...
                                    if (players.get(k).getRole() == SnakesProto.NodeRole.DEPUTY) {
                                        System.out.println("УДАЛИЛИ ДЕПУТИ ИЗЗА СТОЛКН");
                                        for (int s = 0; s < players.size(); s++) {
                                            if (s == k) continue;
                                            if (players.get(s).getRole() == SnakesProto.NodeRole.NORMAL) {
                                                players.add(createGamePlayer(players.get(s).getName(), players.get(s).getId(), players.get(s).getIpAddress(), players.get(s).getPort(), SnakesProto.NodeRole.DEPUTY, players.get(s).getScore()));
                                        ...
                                        for (int s = 0; s < players.size(); s++) {
                                            if (s == k) continue;
                                            System.out.println("МАСТЕР УМЕР ИЗЗА СТОЛКН");
                                            changeServer = true;
                                        }
                                    }

                                    players.remove(k);
                                    System.out.println("УДАЛИЛИ ПЛЕЕРА");
                                    break;
                                }
                            }
                        }
...
//        }
        for (int i = 0; i < players.size(); i++) {
            sendMessage(InetAddress.getByName(players.get(i).getIpAddress()), players.get(i).getPort(), createStateMsg());
        }
    }
